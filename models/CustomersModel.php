...
class CustomersModel extends ModelBase
{
	...
	*******************************************************************************/
     ...
     * @return pdo
     */
    public function getAllCustomersForTasks($id_tenant)
    {
        $consulta = $this->db->prepare("
                        select
                            a.id_customer
                            , a.code_customer
                            , b.id_tenant
                            , CONCAT(a.label_customer, ' | ', a.customer_dni) as label 
                        from cas_customer a
                        inner join cas_tenant b
                        on a.id_tenant = b.id_tenant
                        where b.id_tenant = $id_tenant
     ...
     * @param varchar $customer_dni
     * @return pdo
     */
    public function addNewCustomer($id_customer, $code_customer, $id_tenant, $label_customer, $detail_customer = null, $customer_dni = null)
	{
            $this->db->exec("set names utf8");

            if($customer_dni) {
                $customer_dni = str_replace('.', '', $customer_dni);
            }
	...
	*/
     ...
     * @return pdo
     */
    public function updateCustomer($id_customer, $code_customer, $id_tenant, $label_customer, $detail_customer, $customer_dni = null)
	{
        $this->db->exec("set names utf8");
        ...
        $consulta = $this->db->prepare("UPDATE cas_customer SET label_customer = '".$label_customer."'
                , detail_customer = '".$detail_customer."'
                , customer_dni = '".$customer_dni."'
                WHERE id_customer = ".$id_customer);
            //WHERE id_customer = ".$id_customer." and id_tenant = ".$id_tenant);
