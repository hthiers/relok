<script type="text/javascript">
    ...
    $(document).ready(function(){
      ...
      $("#cbocustomers").change(function(e) {
        ...
        $("#hdnPicker").val(formatDateTimeStringNoTime(date_ini));

        //set timepicker for init time field
        var task_time = "<?php echo $date_ini; ?>";
        $("#hora_ini").val(formatDateTimeStringTime(task_time));

        $('#hora_ini').timepicker({
            'step': 15,
            'scrollDefault': formatDateTimeStringTime(task_time),
            'timeFormat': 'H:i:s'
        });

        //set duration picker
        var task_total = "<?php echo $time_total; ?>";
        if(task_total > 86400){
          task_total = 86400;
        }

        var tiempo_array = secondsToTime(task_total);
        var tiempo_string = tiempo_array['h']+':'+tiempo_array['m']+':'+tiempo_array['s'];

        $('#duration').val(tiempo_string);

        $('#duration').timepicker({
            'step': 15,
            'minTime': '00:15:00',
            'scrollDefault': tiempo_string,
            'timeFormat': 'H:i:s'
        });
    ...
    $(function() {
        ...
        $( "#datepicker" ).datepicker({
            ...
            defaultDate: formatDateTimeStringNoTime("<?php echo $date_ini; ?>")
        });
    });
