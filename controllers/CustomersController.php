    ...
    */
    public function ajaxCustomersDt()
    {
        ...
        */
        ...
        error_log("SQL ajaxCustomersDt: $sql");
...
//                $row[] = utf8_encode($aRow[ $i ]);
    ...
    public function customersAddForm($error_flag = 0)
    {
        ...
        {
            return $this->customersDt(10, $message = "El usuario no cuenta con los permisos para crear un cliente");
        }
    ...
    public function customersAdd()
    {
        ...
        //Le pedimos al modelo todos los items
        $result = $model->addNewCustomer(null, $code_customer, $session->id_tenant, $label_customer, $detail_customer, $customer_dni);
    ...
    public function customersEditForm()
    {
        ...
        if($session->id_profile != 1)
        {
            return $this->customersDt(10, $message = "El usuario no cuenta con los permisos para editar el cliente");
        }
        ...
        if($values != null && $values != false){
            #data
            $customer['id_customer'] = $values['id_customer'];
            $customer['code_customer'] = $values['code_customer'];
            $customer['id_tenant'] = $values['id_tenant'];
            $customer['label_customer'] = $values['label_customer'];
            $customer['detail_customer'] = $values['detail_customer'];
            $customer['customer_dni'] = $values['customer_dni'];
        ...
        $this->view->show("customers_edit.php", $data);
    }
    ...
    public function customerEdit()
    {
        ...
        $customer->setIdCustomer($id_customer);
        $pdoCustomer = $model->getCustomerByID($session->id_tenant, $id_customer);
        $dataCustomer = $pdoCustomer->fetch(PDO::FETCH_ASSOC);
        $customer->setCodeCustomer($dataCustomer['code_customer']);
        $customer->setIdTenant($dataCustomer['id_tenant']);
        $customer->setLabelCustomer(filter_input(INPUT_POST, 'label_customer'));
        $customer->setDetailCustomer(filter_input(INPUT_POST, 'detail_customer'));
        $customer->setCustomerDni(filter_input(INPUT_POST, 'customer_dni'));

        $result = $model->updateCustomer($customer->getIdCustomer(), $customer->getCodeCustomer(), $customer->getIdTenant(), $customer->getLabelCustomer(), $customer->getDetailCustomer(), $customer->getCustomerDni());
        $error = $result->errorInfo();
        $rows_n = $result->rowCount();
        ...
        if($error[0] == 00000){
            ...
            header("Location: ".$this->root."?controller=".$controller."&action=".$action."&error_flag=10&message='Ha ocurrido un error: ".$error[2]." ___ ".$id_customer."'");
        }
