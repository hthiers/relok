...
class AdmtypesController extends ControllerBase
{
    /*******************************************************************************
    * TYPES
    *******************************************************************************/
    ...
    public function typesDt($error_flag = 0, $message = "")
    {
        ...
        require_once 'models/AdminmanagementsModel.php';
        ...
        $listadoClientes = $clientModel->getAllManagements($session->id_tenant);
        ...
        $this->view->show("admintype_dt.php", $data);
...
//                $row[] = utf8_encode($aRow[ $i ]);
    ...
    public function ajaxTypesUpdate()
    {
        ...
        if(isset($_POST['row_id']) && $_POST['row_id'] != ""):
            ...
            //Creamos una instancia de nuestro "modelo"
            $model = new TypesModel();
            $targetTypePdo = $model->getManagementByID($session->id_tenant, filter_input(INPUT_POST, 'row_id'));
            ...
            if($targetType != null && $targetType != false){
                ...
                $result = $model->updateManagement(
                        $targetType['id_management']
                        , $targetType['code_management']
    ...
    public function typesRemove()
    {
        ...
        // Support POST & GET
        if(filter_input(INPUT_POST, 'managementid') != ''){
        ...
        if($id_management != null){
            ...
            if($result != null){
                ...
                if($error[0] == 00000 && $numr > 0){
                    header("Location: ".$this->root."?controller=admtypes&action=typesDt&error_flag=1'");
                }
                else{
                    header("Location: ".$this->root."?controller=admtypes&action=typesDt&error_flag=10&message='No se lograron aplicar cambios: ".$error[2]."'");
                }
            }
            else{
                header("Location: ".$this->root."?controller=admtypes&action=typesDt&error_flag=10&message='Error: no se ha podido eliminar!");
            }
        }
        else{
            header("Location: ".$this->root."?controller=admtypes&action=typesDt&error_flag=10&message='Error: esta materia ya no existe!");
        }
    }
    ...
    public function ajaxUpdateType()
    {
        ...
        $result = $model->updateAdminManagment($id_management, $target_column, $newValue);
