...
//                $row[] = utf8_encode($aRow[ $i ]);
    ...
    public function ajaxManagementsAdd()
    {
        ...
        if(isset($_POST['label_management']) && $_POST['label_management'] != ""):
            ...
            // Estado = 0 para nueva gestión
            $resultPdo = $model->addNewManagement(null, $code_management, $session->id_tenant, $label_management, 0);
            ...
            elseif($error[0] == 00000 && $rows_n < 1){
                $new_management[0] = "0";
                $new_management[1] = "No se ha podido ingresar el registro: ".$error[2];
            }
            else{
                $new_management[0] = "0";
